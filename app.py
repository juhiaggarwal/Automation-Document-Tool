...
def upload_to_cloudinary(file_path):
    try:
        print("📤 Uploading to Cloudinary:", file_path)
        print("🌐 Using cloud:", cloudinary.config().cloud_name)
...
if app.config.get("GITHUB_CLIENT_ID") and app.config.get("GITHUB_CLIENT_SECRET"):
    ...
    scope="user:email"  # 🔑 Request access to email list
)
...
def profile():
    ...
    if request.method == 'POST':
        ...
        if file and file.filename:
            ...
            cloud_url = upload_to_cloudinary(temp_file_path)
    ...
    feedbacks_count = user.feedback_given or 0
    ...
    return render_template(
        ...
        profile_pic=user.profile_pic or url_for('static', filename='default.jpg')
    )
...
def register_user():
    ...
    user = User(
        ...
        original_password=password  # for internal use only
    )
    db.session.add(user)
    db.session.commit()
...
def reset_password():
    ...
    user = User.query.filter_by(email=email).first()
    if not user:
        return jsonify({"success": False, "message": "❌ This email is not registered with us."}), 400
...
def google_auth_complete():
    ...
    if not user:
        ...
        # ✅ Send welcome email
        try:
            msg = Message("Welcome to the Automation Tool", recipients=[email])
            msg.body = f"""Hello {user.first_name or user.email},

Thank you for registering on our Web Application Automation Documentation Tool.
Your registered email ID is: {email}

Regards,
Automation Tool Team
"""
            mail.send(msg)
        except Exception as e:
...
def github_auth_complete():
    ...
    # 2️⃣ If GitHub email is not public, get it from /user/emails
    if not email:
        ...
        if email_resp.ok:
            ...
            if verified_primary:
                email = verified_primary[0]
        if not email:
            return "⚠️ GitHub email not available", 400
    ...
    # 5️⃣ Only send email on first login
    if first_time:
        try:
            msg = Message("Welcome to the Automation Tool", recipients=[email])
