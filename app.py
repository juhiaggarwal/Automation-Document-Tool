...
app_faq = {
    ...
    "how to submit report": "✅ After generating your report on the Automation Documentation Page, scroll to the bottom and click Submit.",
    "how to export to pdf": "📄 First generate your report fill the data ou want to add in our report and hit the submit button  , then click the Export to PDF button to download it as a PDF file."
}
...
def reset_password():
    ...
    user = User.query.filter_by(email=email).first()
    if not user:      
        return jsonify({"success": False, "message": "This email is not registered. Please register first."})
    ...
    body = f"""
    Hello {full_name if full_name else 'User'},
